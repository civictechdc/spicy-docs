...
| B32 | spicy-docs, then spicy-regs (T08 laws) | One Table III chain walk: `iter_table3_chain(acquire, start, *, max_acts, within)` follows each served page's `next_act` with the stop rules spicy-regs' `build_laws` walk states (no next act, another Congress, does not follow, outside the bound, past the stated release point); absence is established only by the chain, never by page bytes (OLRC answers an unheld act with a 200 site-template prefix, then drops the connection); `retain_dropped_body` keeps the bytes as `response-incomplete` evidence on keyless routes. spicy-regs adopts it at the next re-vendor, counting requests in its `acquire` wrapper. | spicy-regs `build_laws.py` chain walk (2026-09-24); receipts `table3-walk-2026-09-24/` | spicy-regs deletes its walk; a new Congress is seeded from the previous one's last served page (inferred, not yet observed) |
| B33 | spicy-docs | Harden the shared readers to RefSpec's refusals before RefSpec switches to them (measured 2026-09-24 on deliberately broken inputs): `sources/cfr/ecfr.py` accepts 3 of the 4 broken eCFR titles inputs RefSpec's `parse_ecfr_cfr_titles` refuses; the CBO feed reader and RefSpec's `cbo_topic_codes` each refuse cases the other accepts; the XSD reader refuses 2 of the 20 Unified Agenda lists RefSpec reads (`RFA_REQUIRED`, `TTBL_ACTION`); the OpenAPI reader accepts a blank enum member. Move each RefSpec refusal into the spicy-docs reader with the broken input as a fixture; then B21's remaining switches. | RefSpec `wt/repin-0.31.0` PLAN note; survey §11 agreement counts (50/50, 1,058/1,058 on real files) | RefSpec deletes the four copies |
...
| A7 | done: spicy-regs `a934232` (label-aware docket reads through spicy-docs' reader, the literal fallback deleted after it changed no join; unpadded FR keys that refuse different-width padding; folds named in status); rulemaking snapshot `snapshot_21818212…` published 2026-09-24 01:20 UTC — missing FR references 49,403 → 8,036, labelled docket joins 48,169 → 154,943. Follow-up: read labelled docket lists through `normalize_docket_references` (5,807 link rows) | [survey](parsing-survey-2026-09-23.md); receipt `rulemaking-joins-2026-09-23/candidate-0.31.0/` |
...
| A10 | done in spicy-docs 0.31.0 (`d998451`, `0bd70c5`): trailing punctuation stripped, dashes folded, ranges split into both endpoints, "Part" in any case; spicy-regs re-reads `document_citations` under 0.31.0 (rule versions 002/003); the first production re-read (spicy-regs `document_citations`, 2026-09-24) found 8 lost citations (a `Public Law` label glued to a schedule line number; a CFR part before an em dash), GSA prospectus numbers read as RINs and subpart letters repeating a part, fixed in `93baed4` (the `a10-regressions.json` fixture) and adopted by spicy-regs `2ddc313`; the held-parent repair is qualified at `a5d46aa6…` | [survey](parsing-survey-2026-09-23.md) §2, §11; receipt `b4-citation-grammar-2026-09-23/` |
...
| A12 | rulemaking half merged with A7 (`a934232`): one Eastern-day rule through spicy-docs' `regulations_gov_day`, equal to the old rule on all 1,488,432 distinct values of the build's 26 date columns; the bill-walk distinct count came with B6; `rulemaking_lifecycles`, `agency_monthly_volume` and `sources/iceberg.py` stay proposed | [survey](parsing-survey-2026-09-23.md) §11 |
...
| A14 | done: spicy-docs 0.31.0 (`5847505`, `4c67e1e`, `a17fa2f`, `197937c`); spicy-regs `2f6ae9f`/`a8a2554` fetch annual volumes through `acquire_annual` (a refused volume keeps its package's prior rows) and leave the listing's count and identity checks to spicy-docs; 0.31.0 admits all 262 retained volumes where 0.30.0 refused two | [survey](parsing-survey-2026-09-23.md) §11 |
...
| B6 | done: spicy-docs 0.31.0 (`9d53b32`…`61c86c1`): pooled identities across up to four differently paged walks, settled on a clean walk or a pool equal to the declared count, typed `DeclaredCountMismatch`/`DeclaredCountChanged`/`IncompleteWalkError`; spicy-regs `8543ca5` adopts it and deletes `pooled_walk.py` (the typed errors abort before any upload); the Congress index walks (nominations, treaties and the builders sharing `_walk`) now settle by distinct source identity (spicy-regs `604e789`), which serves the four nominations a single-offset walk never returned; `nominations` is re-qualified at `d4622c60…` | [survey](parsing-survey-2026-09-23.md) |
...
| B13 | proposed | §11 |
| B14 | done in RefSpec (`cea465c9`, pushed 2026-09-24): Zyte through `spicy_docs.sources.zyte`, `zyte_transport.py` deleted (205 lines); an empty body is now refused at the ICPSR capture (`c099c455`) | RefSpec `vendor/README.md`; review 2026-09-24 |
...
| B20 | spicy-docs released in 0.31.0 (`b9603a3`): byte-identical to RefSpec's minter on 3,966,225 paired calls except the `-RULE`-family docket suffixes B4 admits; RefSpec, spicy-regs, spicysearch and rulespec-projection adopt at their repins; REF-024 amended in RefSpec `56a76e78` (decision 28, confirmed 2026-09-24) | [survey](parsing-survey-2026-09-23.md) §11 |
| B21 | measured 2026-09-24: RefSpec's lock resolves on 0.31.0 with no conflict; the pin alone fails only the vendor-README digest test (fast tier 6,147 passed; the slow tier's one failure is a pre-existing Atlas digest); every module RefSpec hashes into receipts is byte-identical between the wheels; the eight reader modules B14/B15/B16/B21 import are byte-identical in 0.26.6, so they need no repin. Sequence: (1) done and pushed to RefSpec main 2026-09-24 (`afdbac1f` pin to 0.31.0 and RefSpec dev15, `cea465c9` B14, `9c631d87` B21's PDF page reads through one `pdf_page_texts`, `c099c455` review fixes; fast tier 6,185 passed) — the eCFR, CBO, XSD and OpenAPI switches wait on B33; (2) the import switch for the grammar, shapes and minters — 45 files, the old modules kept as test oracles, three expectations move (`41 CFR 60- 1`, the `-NONRULEMAKING` docket, `_usc_section_order`) and seven Unified Agenda count pins re-adjudicate on a rebuild (800,579 → 800,586 rows); then B22; (3) B11's projection on the same rebuild; (4) B15 at the next scheduled rebuild, B16 at cache regeneration | receipt: the scout's report (this session, 2026-09-24); RefSpec `vendor/README.md`, `src/refspec/registry/{citation_grammar,identifier_shapes,iri_minting,unified_agenda_editions}.py` |
...
| NoChamber | done: `4310647` (0.32.1) passes the publisher's `NoChamber` meeting details through the Congress listing reader; spicy-regs `2ddc313` adopts it and `committee_meetings` is re-qualified at `f85be7b6…` with that population | spicy-regs ledger T08 |
| C1 | proposed | — |
| C2 | proposed | — |
| C3 | not before C1 | — |
