...
| B4 | **Reopened with a target on 2026-09-20** (the owner's rules: roll the PDF families up with the keys that relate them to other tables; never recreate what an index states; leave nothing of value only the PDF holds). The yield measurement (`pdf-family-rollup-yield-2026-09-20.md`, merge 6cea782) verdicted the families; **the MODS re-check (`pdf-yield-mods-recheck-2026-09-20.md`) then superseded its GovInfo rows and its build order** — the activity reports' bills, laws, U.S. Code sections and Statutes pages are all already in the package MODS (0 of 1,406, 1 of 174, 0 of 37, 0 of 7 print-only at full page depth), and the budget volumes, not the activity reports, carry the real citation yield. **The shared contract landed**: `document_citations` (17 columns, one row per occurrence, keyed on the text digest and the span) and `house_activity_reports` (36 columns), thirteen stored citation rules in `interpretation/citations.py` with the measurement tool importing them, and the CRPT MODS's bills, laws, Code sections, CFR parts, Statutes, RINs, committees, submitting member and sibling reports read into `PackageModsIdentity`. The registry is thirty-four contracts over 670 columns. Receipt `document-citations-2026-09-20/`, four keyed requests. Open: the revised build order's steps, and the spicy-regs rollup for both new tables. |
| B8 | Closed by decision: OCR waits for a consumer to name a scanned corpus. |
...
| B4 | CBO estimates sit behind a bot wall; the Court, disclosure sites and activity reports are PDF-only with no target vocabulary | Census; the map's `rejected` rows | **Superseded 2026-09-20.** The remainder was measured family by family and the vocabulary is now named: one shared `document_citations` link table over thirteen cite kinds, plus a per-family document contract where the family earns one. Step 1 landed on the House committee activity reports (`schemas/document_citation_tables.py`, `interpretation/citations.py`); CBO alone stays closed, and for a different reason — no route, not no target | The two contracts through the generic loop, the per-print counts pinned against the rollup sidecar's own numbers, and the MODS-overlap correction asserted in `tests/test_citations.py` over three kinds (bills, laws, U.S. Code sections) rather than the two the first pass compared | spicy-docs contracts, spicy-regs rollup |
