...
| B4, B8 | B4 reopened with a target on 2026-09-20 (the owner: the point of the PDF families is to roll them up with the keys that relate them to other tables; never recreate what an index states; leave nothing of value only the PDF holds): the yield measurement (`pdf-family-rollup-yield-2026-09-20.md`, merge 6cea782, 118 requests, `analyze` reproducible offline) verdicts House activity reports (883 distinct bills, 75 laws, 20 resolved committees across eight prints), the Secretary of the Senate's expenditure tables, GAO recommendations and its report graph, agency uploads, budget volumes and Clerk grids as hosted; CRS's bills discussed and CourtListener's citations already exist so those families contribute only their report graph and body text; CBO stays blocked (the proxy's 520s are transport failures, the census's 403s the refusal; a feed-only bill-to-estimate link is buildable). The Zyte transport joined `spicy_docs.transport`. Build in progress: `document_citations` link contract, the shared citation rules, and `house_activity_reports`. B8 stays deferred. |
