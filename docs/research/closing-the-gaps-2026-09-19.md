...
| B4 | **Reopened and started, 2026-09-20.** The [PDF-family rollup](pdf-family-rollup-yield-2026-09-20.md) measured the remainder and named a target, so the "no schema target exists" ruling no longer holds. Step 1 of its build order landed: `document_citations` (16 columns, one row per occurrence of one cited key, keyed on the span it was read at) and `house_activity_reports` (28 columns), with the nine stored citation rules lifted into `interpretation/citations.py` and the measurement tool importing them. The registry is thirty-four contracts over 661 columns. **The build corrected the measurement it came from**: the package MODS already states every bill and law the activity-report prints name (179 of 179 and 39 of 39 bills on the two packages measured), so the rollup's "883 bills beyond the index" compared against the wrong index record — the `published` listing row, not the MODS the body acquirer already fetches. The yield that survives is the evidence span for those, plus the committees other than the authoring one, the U.S. Code and CFR sections, the Federal Register cites and the GAO and CRS ids. Receipt `document-citations-2026-09-20/`, four keyed requests. Still open: steps 2-7 (GAO and slip opinions into the same table, `crs_report_citations`, `senate_expenditures`, `gao_recommendations`, the budget and Clerk grids, CBO's feed-only link table) and the spicy-regs rollup for both new tables. |
