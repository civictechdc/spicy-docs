...
| A8, A9 | Landed (merge f86715f) after two review rounds: `laws`, `law_code_sections`, `table3_records`, `committees`, `committee_assignments`, the OLRC classification source and the two chamber roster readers, with receipts under `laws-contract-2026-09-19/`, `olrc-classification-2026-09-19/` and `roster-comparison-2026-09-19/`; the registry is thirty-two contracts over 617 columns. The spicy-regs rollups are built on `hosting-rollups-laws-rosters` (laws, code sections and Table III by one rollup, committees and assignments by another; the Statutes at Large citation now fills `congress_bills` by a left join on the published laws at merge time, with the old no-join clause replaced; the committee route's over-declaration of 238 for 236 served read past narrowly; 5 keyed and 10 keyless requests, receipt retained), review in progress. |
