...
| A11 | Built on spicy-regs branch `hosting-a11-backfill`, unmerged, review in progress: the family rollup backfills the 82nd-107th through the shared bill-route reader seam under the same per-run cap, with resume state in two new published own-tables, backfilled rows marked by NULL `schema_version` and columns a zero would lie about left NULL. Proof: the 92nd Congress walked end to end, declared 767, walked 767; 50 detail rows built; 82 keyed requests, receipt retained. |
