...
| C1, C4 | C1 ran: one live bill-summary call, USD 0.00059 per bill, and it found a defect -- the sealed prompt asks for prose and never names the JSON keys its reader requires, so the model's answer was refused and a production run would publish zero summaries; the prompt fix (keys stated once, `PROMPT_VERSION` moved, decision record) is in progress in spicy-docs. C4 not started. |
