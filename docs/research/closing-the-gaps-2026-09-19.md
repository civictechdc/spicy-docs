...
| B4, B8 | B4 reopened with a target on 2026-09-20 (the owner: the point of the PDF families is to roll them up with the keys that relate them to other tables; never recreate what an index states; leave nothing of value only the PDF holds): the yield measurement (`pdf-family-rollup-yield-2026-09-20.md`, merge 6cea782, 118 requests, `analyze` reproducible offline) verdicts the Secretary of the Senate's expenditure tables, GAO recommendations and its report graph, agency uploads, budget volumes and Clerk grids as hosted; CRS's bills discussed already exists so that family contributes only its report graph and body text; CBO stays blocked (the proxy's 520s are transport failures, the census's 403s the refusal; a feed-only bill-to-estimate link is buildable). **Corrected 2026-09-20 by `pdf-yield-mods-recheck-2026-09-20.md` (24 keyed requests): the GovInfo-served families were compared against the `published` listing row, not against the package MODS that `GovInfoBodyAcquirer` fetches for every body it reads.** Restated as print-only against the MODS, the activity reports' 883 bills and 75 laws are **0 and 0** on that sample and **0 of 1,406 and 1 of 174** on a complete 1,249-page read; U.S. Code and Statutes are 0 as well; what survives there is 27 resolved committees beyond the submitting one, 87 RINs and 38 agency dockets. The MODS also states the submitting member's `bioGuideId` (7 of 8) and 374 report-to-report references, so both are ingest, not extraction. The budget volumes take the lead instead: 504 distinct public laws of 518 are print-only on a full read, against 43 of 69 on the capped sample. CourtListener's record does state the outbound opinion edge (`opinions[].cites`), which the rollup had dropped from the record it compared. The Zyte transport joined `spicy_docs.transport`. Build in progress, reordered: `document_citations` on the **budget** volumes first, a MODS loader for the CRPT-stated keys, then the narrowed activity-report print contract; `bodies.py`'s package-id grammar reaches neither `BUDGET-*` nor `GPO-CDOC-*` and must before either is acquired. B8 stays deferred. |
