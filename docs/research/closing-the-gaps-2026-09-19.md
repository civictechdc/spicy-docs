...
| C1a | `classify_sections` publishes no row: the model returns the prompt's brackets | Measured twice live, 2026-09-19 and 2026-09-20 (`c1-classification-2026-09-20/`): `[introduced-in-house\|govinfo\|0]` for `introduced-in-house\|govinfo\|0`. `section_block` writes `[<id>] <heading>`; `CLASSIFICATION_FIELDS` asks for "the section's bracketed id, copied exactly as given below"; `_read_row` requires the bare id | Reword the `sectionId` field so the prompt asks for the id without its brackets, move `section_classification.PROMPT_VERSION` to `v3` and re-pin its digest. An `enum` of the batch's ids on the request would also produce rows and is the wrong fix alone: it makes a badly worded prompt look correct | One keyed call over the same fixture batch producing three `section_classifications` rows, and the committed live-answer fixture still refused under the old wording | spicy-docs |
