...
| D1, D4 | D1 ran on spicy-regs branch `hosting-d1-measured` (review in progress): all thirteen rollups once under their production caps, local output only, 36 tables written, 5,348 keyed requests over the night with a worst rolling hour of 3,680 against the 4,000 ceiling compared to; the dictionary's coverage statements carry the numbers. It found a private-law printing that aborted the whole bill-family run (fixed there, mutation-checked) and an OLRC Table III body that truncates after a 200 (handled by the step-past and stop rules). One run is a cold start only; the second run measures the held-row skips. D4: the hearing-transcript fixture is still a CRPT body under the real CHRG-119hhrg64431 identity, disclosed in `docs/tables.md`. |
...
| D5 | Measured in the D1 run: 5,348 keyed requests over the night, worst rolling hour 3,680 against the 4,000 ceiling; under the cap, so the eight archive listing reads stand. |
