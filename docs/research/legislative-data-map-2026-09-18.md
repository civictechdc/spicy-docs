...
| bills | Amendments listing | `amendment` | `port 4` | api.data.gov key | 97th Congress+; latest 2026-09-18 | 128,800 | replaces `fetchAmendments`; carry the real `amendment.status` (ledger #7) |
...
| committees | Committee reports | `committee-report` (+ `/text`) | `candidate` | api.data.gov key | 103rd Congress+; latest 2026-09-18 | 20,319 | no route in either repo today; BillTrax `committee_reports` rows come only from uploads |
| committees | Hearings | `hearing` | `candidate` | api.data.gov key | 104th Congress+; updated ≥ 2026-09-18 (sort ignored) | 35,483 | serves `hearing_transcripts.text`, a BillTrax table nothing writes today |
...
| nominations | Nominations | `nomination` | `candidate` | api.data.gov key | 97th Congress+; updated ≥ 2026-09-18 (sort ignored) | 45,481 | no coverage anywhere in the repo; API-first over the nine Senate LIS feeds |
...
| communications | House executive communications | `house-communication` (+ detail) | `candidate` | api.data.gov key | 114th Congress+; updated ≥ 2026-09-18 (sort ignored) | 41,709 | typed: isRulemaking, CRA authority, committee referral with systemCode and date, matching requirement, RIN in reportNature (17 of 25 sampled are rulemakings with a RIN, all referred, 21 name a requirement); the RIN resolves in the Federal Register API, so this is the bridge from this repo's regulatory sources to Congress |
...
| Subject | Data | Route | Status | Credential | Coverage | Count / sample | Note |
|---|---|---|---|---|---|---|---|
| bills | BILLSTATUS bulk ZIP | `bulkdata/BILLSTATUS` | `port 6` | none | API 2021+; bulk 108–119 | 172,703 pkgs; 119th zips 52 MB, 18,956 files, zip modified 18-Sep-2026 08:28 | decision 2 (bulk-scope record) first; builds on `reading/zip_archive.py` |
