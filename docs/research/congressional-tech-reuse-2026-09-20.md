...
| Releases | **None** — `gh release list` is empty; no tags on `main` |
...
| Activity | 254 commits. Humans: Alex Gurvich 179 (mostly 2025-08/09), Mike Deeb 33 (2026-07), Evan Tung 18 (2025-12 → 2026-02). **Last human commit 2026-07-25**; every commit since is `github-actions[bot]` refreshing YouTube and CPI data |
| Open work | 23 open issues, 0 open PRs, 1 stale unmerged branch (`78-implement-library-for-data-warehouse`) |
...
3. **Add a `congress` route to `LIST_ROUTES`** so Congress dates and session
   ...
   separate, later question with its own sourcing problem.
