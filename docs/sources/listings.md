...
| SAM.gov entities | Optional registration status and registration-date window | `entityData` | SAM.gov-issued key |
| USAspending recipients | POST body: limit, page, sort, order, award type, optional keyword | `results` | none |
| FCC ECFS proceedings and filings | Explicit date window, limit, offset | `proceeding` / `filing` | api.data.gov key |
...
- SAM.gov needs its own key: the api.data.gov key answered an empty 404 in
  both header and query form on 2026-09-14. The publisher caps deep walks, so
  window by registration date. USAspending's recipient universe exceeded
  eighteen million rows that day; bound the walk. FCC ECFS states no count,
  so an offset walk proves only what it saw; narrow the window rather than
  walk far.
...
`corpora/supply-2026-09-02/receipts/spicyregs-merge-probes-2026-09-14/`. These
