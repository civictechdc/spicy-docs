...
on a single `ListRoute` table in
...
reader method (`CongressListingReader.records(route, url)`). A route's `path`
names its placeholders (`{congress}`, `{chamber}`, `{code}`, `{type}`,
`{number}`); `optional_params` names the trailing ones a caller may omit —
`bill` may omit both `congress` and `bill_type`, but only in that order, so a
bill type without a Congress refuses. `bill_list_url` and `crs_report_list_url`
keep their original names, arguments and behavior as thin aliases over the
same table and builder, so existing callers are unaffected. Sort support is a
measurement, not an assumption: a 2026-09-19 pass over the [legislative data
map](../research/legislative-data-map-2026-09-18.md) Table A found only
`bill`, `amendment`, `summaries`, `committee-report` and `committee` reorder
on `sort=updateDate`; `list_route_url` refuses a `sort` argument on every
other route instead of sending one the publisher would silently ignore.
...
| Route | Path | Records key | Sort honored | Fixture |
| --- | --- | --- | --- | --- |
| `bill` | `bill/{congress}/{type}` | `bills` | yes | `congress-bill-list.json` |
| `crsreport` | `crsreport` | `CRSReports` | no (legacy builder still sends it) | `congress-crsreport-list.json` |
| `amendment` | `amendment/{congress}` | `amendments` | yes | `congress-amendment-list.json` |
| `committee-bills` | `committee/{chamber}/{code}/bills` | `("committee-bills", "bills")` (nested; see below) | no | `congress-committee-bills-list.json` |
| `bill-actions` | `bill/{congress}/{type}/{number}/actions` | `actions` | no | `congress-bill-actions-list.json` |
| `nomination` | `nomination/{congress}` | `nominations` | no | `congress-nomination-list.json` |
| `hearing` | `hearing/{congress}` | `hearings` | no | `congress-hearing-list.json` |
| `committee-report` | `committee-report/{congress}` | `reports` | yes | `congress-committee-report-list.json` |
| `house-communication` | `house-communication/{congress}` | `houseCommunications` | no | `congress-house-communication-list.json` |
