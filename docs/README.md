...
- [Congress bulk bill text](sources/congress-bulk-bills.md): one BILLS folder per Congress, session and bill type from the 113th on, its listing read first and its zip once for the printings a caller keeps.
...
- [Tables](tables.md): the table contracts spicy-regs hosts, each with grain, identity, version column, supplier and per-column descriptions, and the one-pass bill-family build behind them.
...
- [CBO cost estimates](research/cbo-cost-estimate-routes-2026-09-20.md): the routes to the estimate index and text that do not depend on beating cbo.gov's wall · [what landed](research/cbo-cost-estimates-build-2026-09-20.md), re-derived offline from the retained bytes.
- [Remaining data and delivery gaps](research/remaining-gaps-2026-09-21.md): current cross-repository backlog, evidence, priorities and completion checks after validation, repairs and fork setup.
...
- [congressional-tech reuse review](research/congressional-tech-reuse-2026-09-20.md): what the sibling `civictechdc/congressional-tech` monorepo holds, what is worth taking (one committee-to-YouTube-channel crosswalk), and what stays here, with an offline measurement of its Event-ID rule.
