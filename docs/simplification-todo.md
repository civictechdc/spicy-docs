...
- [x] **PAR09 — Remove the replaced implementations.** Track adoption separately
  ...
  **Completed (2026-09-16):** the last named remainder — RefSpec's own U.S. Code
  ...
  remaining receiver family and is tracked in its own plan (partially
  adopted: the wheel pin is current and several readers import SpicyDocs,
  but the module copies still stand).
...
- [x] **U02:** Add explicit USLM sources: selections, locators, native identity validation,
  ...
  scanner moved from the CFR package to `reading/xml.py` so both families share it.
...
`cloudflare.py` is a cache purge, not a fetcher, and stays. Adoption needs
the local copies deleted; SpicyRegs already pins a current wheel (0.26.4),
so the wheel is no longer the blocker.
