...
refusal rather than an absence; byte, entry, and expansion bounds refuse with the capture retained; and a dropped
Table III answer is a retried transport failure, never an absence."""
...
def test_a_table3_page_dropped_mid_body_is_retried_and_read():
    transport = Transport(page(TABLE3_PAGE[:2000], dropped=True), page(TABLE3_PAGE))
    with UsCodeAcquirer(budget=BUDGET, transport=transport) as source:
        acquired = source.acquire_table3_act("1955:360")
    assert (acquired.result.key, acquired.request_count, acquired.capture.body) == ("1955:360", 2, TABLE3_PAGE)
