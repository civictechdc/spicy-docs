...
Five real annual CFR sections, each as the **evidence document** the PDF
...
table's own face and the table stopped reading as a table. These five PDFs are
...
| `CFR-2023-title7-vol1-sec1-313.evidence.json` | 7 CFR 1.313 | GPO's print-shop footer on its page, which read as a table row until `print_shop_footer`, and a subject heading that wraps at a hyphen (`an-` / `swer`) | 203,087 | `14efa689a3d215772452c3bd5c3e6228deee5e62aa0b450447904b33db5c3d94` | 116 |
...
**Every unresolved region in these five files belongs to a neighbouring part,
...
rather than of two documents with each other. The other four sections have no
committed reference; the benchmark scores those against the publisher's XML at
run time.

These fixtures establish behavior for these five sections' shapes. They
