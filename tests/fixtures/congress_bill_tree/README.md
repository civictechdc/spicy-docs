...
| `constructed-bill-divisions-reported.xml` | The engrossed text again, with `Sec. 104` renumbered to `Sec. 402` and relocated into a new Division C, its words byte-identical | The engine's round-2 move pass. Nothing else differs, so the only change the diff can report is a move, and the moved record is appended after the round-1 records — which is what makes the adapter's `sorted(settled, key=round)` alignment guard non-trivial |
