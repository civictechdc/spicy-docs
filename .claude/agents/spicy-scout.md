...
Read the code, understand which layer owns the fact, name the root, and make one targeted change. When a count or list is wanted, read the live source (`filemap.json`, the directory) rather than trusting prose — prose counts decay.
