...
- [document_capture](analysis/document_capture.py): convert six real federal
  ...
  against Rulespec's parent schema and its family profile, prove the text round
  trip with a second parser, and render two leaves per document as rulespec
  `SourceFragment`s. Reads committed fixtures and the retained inputs beside its
  output; makes no request. Supply `--output`; the design record is
