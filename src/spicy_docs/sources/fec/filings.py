...
def _validate_options(encoding, max_record_bytes):
...
def filing_records(
    *, store: Path, sha256: str, encoding: str = "utf-8", max_record_bytes: int = 1024**2
    ...
    """
    _validate_options(encoding, max_record_bytes)
    with LocalBlobSource(store).open(sha256) as stream:
        yield from filing_records_from_stream(
            stream, sha256=sha256, encoding=encoding, max_record_bytes=max_record_bytes
        )


def filing_records_from_stream(stream, *, sha256, encoding="utf-8", max_record_bytes=1024**2):
    """Parse a caller-owned verified binary stream from offset zero without reopening it.

    The caller owns verification and closure; source ranges refer to this decoded
    stream. For ZIP members retain the container pin and member ordinal separately.
    """
    _validate_options(encoding, max_record_bytes)
